...
class AmulAPIClient:
    ...
    def get_products(self) -> List[Dict[str, Any]]:
        """Fetch a specific product from Amul API by alias."""
        ...
        params = {
            ...
            "q": json.dumps({"alias": "amul-high-protein-rose-lassi-200-ml-or-pack-of-30"}), # Set specific product alias
            "limit": 1 # Limit to one product
        }
...
def main(force: bool, dry_run: bool) -> None:
    ...
    # 1. Initial Force Send when the script is initialized
    logger.info("Performing initial force send notification upon startup...")
    try:
        # Note: The `force` CLI argument will only apply to this initial run
        # if `--force` is used. Otherwise, it defaults to Config.FORCE_NOTIFY.
        # However, for the "initial force send" requirement, we explicitly set it to True here.
        checker.run(force_notify=True, dry_run=dry_run)
        logger.info("Initial force send complete.")
    except Exception as e:
        logger.error(f"Error during initial force send: {e}")

    last_force_send_time = time.time() # Record time of initial force send
    FOUR_HOURS_IN_SECONDS = 4 * 60 * 60 # This remains 4 hours for the scheduled force send

    # 2. Main loop for regular checks and scheduled force sends
    while True:
        # Perform the regular check for newly available products (every 10 seconds)
        # This uses your Redis state management logic (unless Redis is unavailable)
        try:
            logger.info("Starting regular product availability check (every 10 seconds)...")
            # Explicitly set force_notify=False for the regular check
            checker.run(force_notify=False, dry_run=dry_run)
            logger.info("Regular check complete.")
        except Exception as e:
            logger.error(f"Error during regular check: {e}")

        # Check if 4 hours have passed for the scheduled force send
        current_time = time.time()
        if current_time - last_force_send_time >= FOUR_HOURS_IN_SECONDS:
            logger.info("4 hours elapsed. Performing scheduled force send notification...")
            try:
                checker.run(force_notify=True, dry_run=dry_run) # Force send this time
                logger.info("Scheduled force send complete.")
                last_force_send_time = current_time # Reset the timer
            except Exception as e:
                logger.error(f"Error during scheduled force send: {e}")
        else:
            minutes_remaining = int((FOUR_HOURS_IN_SECONDS - (current_time - last_force_send_time)) / 60)
            logger.info(f"Next scheduled force send in approx. {minutes_remaining} minutes.")

        logger.info("Waiting for 10 seconds before next check...")
        time.sleep(10) # CHANGED: Wait for 10 seconds here
